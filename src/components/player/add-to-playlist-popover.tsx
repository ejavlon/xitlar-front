...
export function AddToPlaylistPopover({
  ...
  // Fetch user personal playlists dynamically when open
  useEffect(() => {
  ...
  // Update checkbox state whenever track changes or playlists change
  useEffect(() => {
    if (!track) {
    ...
    playlists.forEach((p) => {
      stateMap[p.id] = Boolean(p.tracks?.some((t) => String(t.id) === String(track.id)));
    });
  ...
  const showFeedback = (text: string, type: "add" | "remove") => {
    if (feedbackTimerRef.current) {
      clearTimeout(feedbackTimerRef.current);
    }
  ...
  const handleTogglePlaylist = async (playlistId: string) => {
    ...
    const targetPlaylist = playlists.find((p) => String(p.id) === String(playlistId));
    const playlistTitle = targetPlaylist?.title || "playlist";
    ...
    try {
      ...
      // Update selectedPlaylists map
      setSelectedPlaylists((prev) => ({
        ...prev,
        [playlistId]: nextChecked
      }));

      // Update local playlists state with updated track list
      setPlaylists((prev) =>
        prev.map((p) => {
          if (String(p.id) !== String(playlistId)) return p;
          const currentTracks = p.tracks || [];
          const updatedTracks = nextChecked
            ? [...currentTracks.filter((t) => String(t.id) !== String(track.id)), track]
            : currentTracks.filter((t) => String(t.id) !== String(track.id));
          return { ...p, tracks: updatedTracks, trackCount: updatedTracks.length };
        })
      );

      // Show feedback card
      const msg = nextChecked
  ...
  const handleCreatePlaylist = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append(
        "data",
        new Blob([JSON.stringify({ title, description: "User playlist" })], {
      ...
      if (newPlResponse && newPlResponse.id) {
        ...
        const newPl: Playlist = {
          ...
          trackCount: 1,
          isCollection: false,
          creator: newPlResponse.createdBy
            ? `${newPlResponse.createdBy.firstName || ""} ${newPlResponse.createdBy.lastName || ""}`.trim() ||
              newPlResponse.createdBy.username
            : undefined,
          tracks: [track]
        };
  ...
  return (
    <div ref={containerRef} className={cn("relative inline-flex items-center", className)}>
      ...
      {isOpen && track && (
        !isAuthorized ? (
          /* Guest / Unauthorized Prompt Card */
          <div
            onClick={(e) => e.stopPropagation()}
            className={cn(
              "w-[210px] bg-[#456690] text-white rounded-xl shadow-2xl overflow-hidden z-50 animate-fade-in select-none p-4 text-center border border-white/10 relative",
              position === "top"
          ...
          >
            {/* Pointer Arrow */}
            {position === "top" ? (
              <div className="absolute -bottom-1.5 right-4 w-0 h-0 border-x-6 border-x-transparent border-t-6 border-t-[#456690]" />
            ) : (
              <div className="absolute -top-1.5 right-3 w-0 h-0 border-x-6 border-x-transparent border-b-6 border-b-[#456690]" />
            )}

            <p className="text-xs font-semibold leading-relaxed mb-3 text-white">
            ...
            <Link
              ...
              className="inline-block bg-[#ffea60] hover:bg-[#ebd33a] text-slate-900 font-bold px-6 py-1.5 rounded-full text-xs transition-colors shadow-xs"
            >
              Войти
            </Link>
          </div>
        ) : (
          /* Authorized User Playlist Selection Popover */
          <div
            onClick={(e) => e.stopPropagation()}
            className={cn(
          ...
          >
            {/* 1. Header (Brand Blue with Title and Close Button) */}
            ...
            {/* Content: Either Feedback Confirmation or Playlist Selection */}
            {feedback ? (
              /* Feedback Confirmation Screen (Image 2 style) */
              <div className="p-6 flex flex-col items-center justify-center text-center animate-fade-in">
                <div className="w-12 h-12 rounded-full bg-[#0091ff] flex items-center justify-center mb-3 shadow-md">
                  <Check className="w-7 h-7 text-black stroke-[3]" />
                </div>
                <p className="text-xs font-medium text-slate-800 leading-snug px-1">
                  {feedback.text}
                </p>
              </div>
            ) : (
              /* Playlists List & Actions */
              <>
